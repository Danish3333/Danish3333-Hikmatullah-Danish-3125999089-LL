...
def batch_calculate_runoff(P_values: Union[List[float], np.ndarray],
                            CN: float) -> np.ndarray:
    """Vectorized SCS-CN calculation for multiple rainfall values."""
    P_arr = np.asarray(P_values, dtype=float)
    S = calculate_S(CN)
    Ia = calculate_Ia(CN)
    Q = np.where(P_arr > Ia, (P_arr - Ia) ** 2 / (P_arr - Ia + S), 0.0)
    return np.minimum(Q, P_arr)


# Example calculation
if __name__ == '__main__':
    # Test with known example: P=50mm, CN=80 -> Q=13.8mm
    P, CN = 50, 80
    S = calculate_S(CN)
    Ia = calculate_Ia(CN)
    ...
    print(f"  Ia = {Ia:.1f} mm")
    print(f"  Q = {Q:.1f} mm")
    print(f"  Verification: Q <= P? {'Yes' if Q <= P else 'No'}")